...
class Monkey {
  ...
  addItem(item) {
    this.items.push(item % this.totalModulo)
  }

  updateModulo(mod) {
    this.totalModulo = mod
  }
  ...
  playWithItem() {
    ...
    const worryLevel = computedValue
...
class PartyOfMonkeys {
  constructor(inputText) {
    ...
    for (const monkeyNumber in this.monkeys) {
      ...
      totalModulo *= currentMonkey.tests.divisible
    }


    for (const monkeyNumber in this.monkeys) {
      const currentMonkey = this.monkeys[monkeyNumber]
      currentMonkey.updateModulo(totalModulo)
    }
...
// console.log(monkeys.monkeys[0])
// console.log(monkeys.monkeys[1])
// console.log(monkeys.monkeys[2])
